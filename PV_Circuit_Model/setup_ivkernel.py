...
with silent(silent_build):
    ...
    # --------------------------------------------------------------------
    pyx_file = here / "ivkernel_wrapper.pyx"
    cpp_file = here / "ivkernel_wrapper.cpp"
    kernel_cpp_file = here / "ivkernel.cpp"
    ...
    sources = [str(cpp_file), str(kernel_cpp_file)]
    ...
    if os.path.exists("PV_Circuit_Model"): # meaning we're outside "PV Circuit Model"
        try_path = "PV_Circuit_Model.ivkernel"
    else:
        try_path = "ivkernel"
    ...
    try:
        ...
        setup(name="ivkernel",ext_modules=ext_modules)
    except Exception as ex:
        build_error = ex
        
if build_error is not None:        
    print("Build ivkernel failed:", build_error)
